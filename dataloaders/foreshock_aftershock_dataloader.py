...
def prepare_foreshock_aftershock_dataloaders(
  *,
  ...
  # CROP SAMPLES such that the length is divisible by 16
  # TODO: remove hardcoded sample_length
  X_train, y_train = datasets["train"]["X"][:, :2496, :], datasets["train"]["y"]
  X_val, y_val = datasets["val"]["X"][:, :2496, :], datasets["val"]["y"]
  X_test, y_test = datasets["test"]["X"][:, :2496, :], datasets["test"]["y"]
